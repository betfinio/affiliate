...
const EarningOverview: FC = () => {
	...
	return (<div className={'flex flex-col gap-2 md:gap-3 lg:gap-4'}>
						...
						<tr className={'text-gray-500 text-left font-medium'}>
							...
							<th className={'pb-3 text-right font-medium '}>
								<div className={'flex flex-row items-center justify-end gap-1'}>
									<DirectAffiliate className={'w-4 h-4'}/>Direct <span className={'hidden md:block'}>bonus</span>
								</div>
							</th>
							<th className={'pb-3 text-right font-medium'}>
								<div className={'flex flex-row items-center justify-end gap-1'}>
									<DepthAffiliate className={'w-4 h-4'}/>Matching <span className={'hidden md:block'}>bonus</span>
								</div>
							</th>
						</tr>
						</thead>
						...
						<tr>
							<td className={'text-left  py-2'}>
								<div className={'flex flex-row items-center gap-1'}>
									<Square3Stack3DIcon className={'w-4 h-4 text-yellow-400'}/>{t('table.staking')}
								</div>
							</td>
							...
							<td rowSpan={2} className={cx('text-right py-2', {'animate-pulse blur-sm': !balance})}>
								<div className={'flex justify-end'}>
									<BetValue
										value={valueToNumber(!balance ? 12345n * 10n ** 18n : balance.matching.total)} withIcon
										precision={2} place={'right'}/>
								</div>
							</td>
						</tr>
						<tr>
							<td className={'text-left  py-2'}>
								<div className={'flex flex-row items-center gap-1'}>
									<Blackjack color={'white'} className={'w-4 h-4 text-yellow-400'}/>{t('table.bets')}
								</div>
							</td>
							<td className={cx('text-right py-2 flex justify-end', {'animate-pulse blur-sm': !balance})}>
								<BetValue value={valueToNumber(!balance ? 12345n * 10n ** 18n : balance.bets.total)}
								          withIcon precision={2} place={'right'}/>
							</td>
						</tr>
						</tbody>
					</table>
			...
			<div style={{backgroundImage: 'radial-gradient(circle, #201C40, rgba(115, 102, 255, 0.5) 0%, #201C40 50%)'}}
				...
				<div>
					<h1 className={'text-sm md:text-xl text-gray-300 flex gap-1 items-center'}>
						...
						{t("claimDirect")}:
						{!balance ? <Skeleton className={'w-16 h-6'}/> :
				...
				<div className={' flex flex-col gap-1 md:gap-5 justify-between '}>
					<div>
						<h1 className={'text-sm md:text-xl text-gray-300  flex flex-row items-center gap-1'}>
							...
							{t('claimMatching')}:
							<BetValue className={'text-white font-semibold'}
							          value={valueToNumber(!balance ? 0n : balance.matching.claimable)} withIcon/>
						</h1>
						<div className={'text-gray-500 text-xs mt-1 flex items-start gap-1'}>
							...
							<TooltipProvider delayDuration={0}>
								<Tooltip>
									<TooltipTrigger>
								<span className={'border text-yellow-500 border-yellow-500 font-semibold text-[10px] w-[14px] h-[14px] flex items-center justify-center rounded-full'}>
									?
								</span>
									</TooltipTrigger>
									<TooltipContent
										...
										<div className={'px-8 py-5 text-xs max-w-[90vw] leading-5'}>
											...
											<a className={'font-semibold underline text-yellow-400'} href={getStakingUrl()}>staking</a> volume or
						...
						<div className={'flex'}>
							<div className={'rounded-lg p-2 bg-yellow-400 bg-opacity-15 flex  gap-3 items-center mt-2'}>
								<div className={'flex flex-row gap-1 items-center'}>
									+<BetValue value={valueToNumber(pending)} className={'font-semibold'} withIcon
									           iconClassName={'w-[12px] h-[12px]'}/> in
