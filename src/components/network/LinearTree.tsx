...
import Tree, {CustomNodeElementProps, Point, RawNodeDatum, TreeNodeDatum} from "react-d3-tree";
import cx from "clsx";
		...
		<div className={' border border-gray-800 rounded-md mt-2 md:mt-3 lg:mt-4 h-[80vh] relative'} ref={boxRef}>
			...
			<FullScreen handle={handle}>
				{handle.active && <>
					...
					<Tree data={data} onUpdate={handleUpdate} pathFunc={"step"}
					      ...
					      renderCustomNodeElement={renderElement} orientation={'horizontal'}/></>}
			</FullScreen>
			<Tree data={data} onUpdate={handleUpdate} pathFunc={"step"}
			      nodeSize={getNodeSize()}
			      zoomable={true}
