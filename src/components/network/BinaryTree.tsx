...
import Tree, {CustomNodeElementProps, Point, RawNodeDatum, TreeLinkDatum, TreeNodeDatum} from "react-d3-tree";
import cx from "clsx";
...
const BinaryTree = () => {
	...
	const expand = async (member: string, lvl: number = level) => {
		...
		if (right.data) childrenData.push(right.data.member); else if (left.data !== null) {
	...
	const updateLevel = (lvl: number) => {
		...
		toast({
			text: "Expand more",
			description: "Click on + icon below the member whose structure you want to display",
			variant: "default"
		})
	}
	...
	return <div>
		<div className={'text-center text-xs font-semibold text-gray-500 italic px-5'}>Linear and binary view are two
		...
		<div className={' border border-gray-800 rounded-md mt-2 md:mt-3 lg:mt-4 h-[80vh] relative'} ref={boxRef}>
			...
			<FullScreen handle={handle}>
				{handle.active && <>
					...
					<Tree data={data} onUpdate={handleUpdate} pathFunc={"straight"}
					      ...
					      renderCustomNodeElement={renderElement} orientation={'vertical'}/></>}
			</FullScreen>
			<Tree data={data} onUpdate={handleUpdate} pathFunc={"straight"}
			      nodeSize={getNodeSize()}
			      zoomable={true}
