...
const LinearTable = () => {
	...
	const columnsWithActions = [...getColumns(path.length - 1), columnHelper.display({
		id: 'action',
		meta: {
			className: 'w-10'
		},
			...
			return (
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						...
						{row.original.direct_count > 0 && <DropdownMenuItem onClick={() => handleExpandMember(row.original.member)}>
							Expand member
						</DropdownMenuItem>}
					</DropdownMenuContent>
				</DropdownMenu>
	...
	return <Breadcrumb>
		<BreadcrumbList className={'p-2'}>
			<BreadcrumbItem className={'cursor-pointer bg-yellow-400 text-black rounded-md px-2 '} onClick={() => onChange([path[0]])}>
				<UsernameOrAddress member={path[0]}/>(me)
			</BreadcrumbItem>
			{path.length > 3 &&
				<>
					<BreadcrumbSeparator/>
					<BreadcrumbItem>
						<DropdownMenu>
							...
							<DropdownMenuContent align="start">
								{path.slice(1, path.length - 1).reverse().map((member, index) => (
